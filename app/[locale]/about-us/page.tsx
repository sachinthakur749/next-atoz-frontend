...
export async function generateMetadata(): Promise<Metadata> {
  const pageData = await getRequest("content/page?slug=about-us");
...
const page = async () => {
  const aboutData = await getRequest("content/page?slug=about-us");
  ...
  return <>
    ...
    <div className="">
      <BannerContainer
        title={aboutData?.data?.title ?? "About US"}
          ...
          <div className="w-full md:w-[40%] h-max">
            <div className="relative h-[350px] w-full overflow-hidden rounded-[5px]">
              <Image
                ...
                alt={aboutData?.data?.altTag_2 || "About us"}
                fill
